...
<script>
    export default {
        data() {
            return {
                mentor: {},
                token : { headers: {'Authorization': `Bearer ${localStorage.getItem('my-app-token')}`} }
            }
        },
        created() {
            this.axios
                .get(`http://localhost:8000/api/mentor/edit/${this.$route.params.id}`, this.token)
                .then((response) => {
        ...
        methods: {
            updateMentor() {
                this.axios
                    .post(`http://localhost:8000/api/mentor/update/${this.$route.params.id}`, this.mentor, this.token)
                    .then((response) => {
